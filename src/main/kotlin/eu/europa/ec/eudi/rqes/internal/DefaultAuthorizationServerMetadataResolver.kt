...
import eu.europa.ec.eudi.rqes.AuthorizationServerMetadataResolutionException
import eu.europa.ec.eudi.rqes.AuthorizationServerMetadataResolver
import eu.europa.ec.eudi.rqes.CSCAuthorizationServerMetadata
...
internal class DefaultAuthorizationServerMetadataResolver(
    ...
    override suspend fun resolve(authServerUrl: HttpsUrl): Result<CSCAuthorizationServerMetadata> =
    ...
    /**
     * Tries to fetch the [CSCAuthorizationServerMetadata] for the provided [OpenID Connect Authorization Server][issuer].
     * The well-known location __/.well-known/openid-configuration__ is used.
     */
    private suspend fun fetchOidcServerMetadata(issuer: HttpsUrl): Result<CSCAuthorizationServerMetadata> =
        runCatching {
    ...
    /**
     * Tries to fetch the [CSCAuthorizationServerMetadata] for the provided [OAuth2 Authorization Server][issuer].
     * The well-known location __/.well-known/oauth-authorization-server__ is used.
     */
    private suspend fun fetchOauthServerMetadata(issuer: HttpsUrl): Result<CSCAuthorizationServerMetadata> =
        runCatching {
            val url = issuer.metadataUrl(
...
/**
 * Verifies the issuer of this [CSCAuthorizationServerMetadata] equals the [expected] one.
 */
private fun CSCAuthorizationServerMetadata.expectIssuer(expected: HttpsUrl) =
